...
if ($data && $data['Atualiza']):
    ...
    if ($cadastra->getResult()):
        ...
        header('Location: painel.php?exe=animal/index&bold='.$Id);
    else:
        WSErro($cadastra->getError()[0], $cadastra->getError()[1]);
    endif;
else:
    $ReadUser = new Read;
    $ReadUser->ExeRead("app_animal", "WHERE id = :id", "id={$Id}");
    if (!$ReadUser->getResult()):
        Message::FlashMsg("msgAlert", WS_ERROR ,"Animal não encontrado", true);
        header('Location: painel.php?exe=animal/index');
    else:
...
<div class="row row-centered">
    <div class="col-lg-12" id="crop-avatar">
        ...
        <div class="avatar-view" title="">
            ...
            <img src="../<?php if (!empty($data['animal_img'])) {
        ...
        <form class="form-horizontal" method="post" name="form_tipoevento" enctype="multipart/form-data">
            <fieldset>
                ...
                <div class="col-md-6">
                    <label for="basic-url">Cod do Pelo</label>
                    <input type="text" class="form-control" maxlength="11" name="cor_do_pelo"
                           value="<?php if (isset($data['cor_do_pelo'])) {
                               echo $data['cor_do_pelo'];
                           } ?>">
                </div>
                <div class="col-md-6">
                    <label for="basic-url">Tamanho</label>
                    <input type="text" class="form-control" maxlength="11" name="tamanho"
                           value="<?php if (isset($data['tamanho'])) {
                               echo $data['tamanho'];
                           } ?>">
                </div>
