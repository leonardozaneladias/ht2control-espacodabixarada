...
<div class="row row-centered">
    <div class="col-lg-12" id="crop-avatar">
        ...
        if ($data && $data['Atualiza']):
            unset($data['Atualiza']);
            if(!$userId)$userId=$_SESSION['userlogin']['user_id'];
        ...
        <div class="avatar-view" title="">
            ...
            <img src="../<?php if (!empty($data['user_img'])) {
                echo $data['user_img'];
            } else {
            ...
            <fieldset>
                <div class="col-lg-6">
                    ...
                    <input required type="text" class="form-control" name="user_name"
                           value="<?= Form::Value($data['user_name']) ?>">
                </div>
                <div class="col-lg-6">
                    <label for="basic-url">Sobrenome</label>
                    <input required type="text" class="form-control" name="user_lastname"
                           value="<?= Form::Value($data['user_lastname']) ?>">
                </div>
                <div class="col-lg-6">
                    <label for="basic-url">E-mail</label>
                    <input required type="text" class="form-control" name="user_email"
                           value="<?= Form::Value($data['user_email']) ?>">
                </div>
                <div class="col-lg-6">
                    <label for="basic-url">Telefone</label>
                    <input required type="tel" class="form-control" name="user_tel" maxlength="13"
                           value="<?= Form::Value($data['user_tel']) ?>">
                </div>
                <div class="col-lg-12">
                    <label for="basic-url">Tocar senha</label>
                    <input maxlength="12" type="password" class="form-control" name="user_password"
                           value="<?php if (isset($data['user_password'])) {
                               Form::Value($data['user_password']);
                           } ?>"
                ...
                <input type="hidden" name="user_img" id="img_crop" value="<?php if (!empty($data['user_img'])) {
                    echo $data['user_img'];
                } ?>">
