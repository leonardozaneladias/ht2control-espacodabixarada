...
class AdminCliente
{
    ...
    {
        $this->Data = $Data;
        if(isset($this->Data['cliente_img']) && !empty($this->Data['cliente_img'])){
            $this->Data['cliente_img'] = str_replace("../../../", "", $this->Data['cliente_img']);
        }


        foreach ($this->Data as $key => $dados):
            if($key == 'cliente_complemento'): continue; endif;
            if($key == 'cliente_img'): continue; endif;
        ...
        if (isset($Error) && $Error == true):
            if (empty($this->Data['cliente_id'])):
                $this->Result = false;
                $this->Error = ['<b>Erro ao cadastrar:</b> Para cadastrar, adicione uma imagem e preencha todos os campos!', WS_ALERT];
            else:
                $this->Result = false;
    ...
    public function ExeUpdate($CategoryId, array $Data)
    {
        ...
        if (isset($Error) && $Error == true):
            $this->Result = false;
            $this->Error = ["<b>Erro ao atualizar:</b> Para atualizar a {$this->Data['cliente_nome']}, preencha todos os campos!", WS_ALERT];
        else:
            $this->setData();
            $this->setName();
            $this->Update();
    ...
    public function ExeDelete($CategoriaId)
    {
        ...
        $delete = new Delete;
        $delete->ExeDelete(self::Entity, "WHERE cliente_id = :deletaid", "deletaid={$this->CatId}");
        $this->Result = true;
    ...
    {
        $Update = new Update;
        $Update->ExeUpdate(self::Entity, $this->Data, "WHERE cliente_id = :catid", "catid={$this->CatId}");
        if ($Update->getResult()):
            $this->Result = true;
        endif;
    }
