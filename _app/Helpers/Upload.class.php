...
class Upload {
    ...
    /**
     * <b>Enviar Imagem Tmp:</b> Basta envelopar um $_FILES de uma imagem e caso queira um nome e uma largura personalizada.
     * Caso não informe a largura será 1024!
     * @param FILES $Image = Enviar envelope de $_FILES (JPG ou PNG)
     * @param STRING $Name = Nome da imagems ( ou do artigo )
     * @param INT $Width = Largura da imagem ( 1024 padrão )
     * @param STRING $Folder = Pasta personalizada
     */
    public function ImageTmp(array $Image, $Name = null, $Width = null, $Folder = null) {
        $this->File = $Image;
        $this->Name = ( (string) $Name ? $Name : substr($Image['name'], 0, strrpos($Image['name'], '.')) );
        $this->Width = ( (int) $Width ? $Width : 1024 );
        $this->Folder = ( (string) $Folder ? $Folder : 'tmp' );
    ...
    /**
     * <b>Crop Imagem:</b> Basta envelopar um $_FILES de uma imagem e caso queira um nome e uma largura personalizada.
     * Caso não informe a largura será 1024!
     * @param FILES $Image = Enviar envelope de $_FILES (JPG ou PNG)
     * @param STRING $Name = Nome da imagems ( ou do artigo )
     * @param INT $Width = Largura da imagem ( 1024 padrão )
     * @param STRING $Folder = Pasta personalizada
     */
    public function CropImagem($Image, $Name = null, $Width = null, $X, $Y, $W, $H) {
        ...
        $this->File['tmp_name'] = $Image;
        $this->File['name'] = $Image;
        $ImgTmpU = getimagesize($Image);
        $this->File['type'] = $ImgTmpU['mime'];
        $this->Name = ( (string) $Name ? $Name : substr($Image, 0, strrpos($Image, '.')) );
        $this->Width = ( (int) $Width ? $Width : 1024 );
        $this->Folder = ( (string) isset($Folder) ? $Folder : 'images' );

        $this->CheckFolder($this->Folder);
        $this->setFileName();
        $this->UploadCropImage();
    }
    ...
    private function setFileName() {
        ...
        $FileName = (isset($this->File['name']) ? $this->File['name'] : $this->Name );
        $FileName = Check::Name($this->Name) . strrchr($FileName, '.');
    ...
    private function UploadImage() {
        ...
        if (!$this->Image):
            ...
            switch ($this->File['type']):
                ...
                case 'image/jpeg':
                case 'image/pjpeg':
                    imagejpeg($NewImage, self::$BaseDir . $this->Send . $this->Name, 90);
                    break;
                case 'image/png':
                case 'image/x-png':
    ...
    // Crop na Imagem
    private function UploadCropImage() {
        ...
        if (!$this->Image):
            ...
            $NewImage = imagecreatetruecolor($ImageX, $ImageX);
            imagealphablending($NewImage, false);
            imagesavealpha($NewImage, true);
            imagecopyresampled($NewImage, $this->Image, 0, 0, $this->X, $this->Y, $ImageX, $ImageX, $this->W, $this->H);
